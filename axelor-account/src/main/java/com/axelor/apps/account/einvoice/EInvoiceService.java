...
public class EInvoiceService {
    ...
    if (instance == null) {
      ...
      String endpoint =
          System.getProperties()
              .getProperty("einvoice.endpoint", "https://prelive.finbite.eu/finance/erp/");
      ...
      System.out.println("Before port assign value: port == null" + (port == null));
      port = new ErpDataExchangeService().getErpDataExchangeSoap11();
      System.out.println("After port assign value: port == null" + (port == null));
  ...
  public boolean isPartnerAcceptEinvoice(String reg) {
    if (port == null) {
      ...
      port = new ErpDataExchangeService().getErpDataExchangeSoap11();
      System.out.println("After port assign value: port == null" + (port == null));
      getInstance(); // ensure it's initialized
    }
  ...
  public void sendInvoice(com.axelor.apps.account.db.Invoice axelorInvoice) {
    ...
    System.out.println("Sending e-invoice: Invoice with id: " + invoiceId);
    System.out.println("Sending e-invoice: Einvoice port route to: " + "port=" + (port == null) + ", apiKey: " + apiKey);
  ...
  private InvoiceParties invoiceParties(
      Partner seller, Partner buyer, BankDetails bankDetails, Partner contactPerson) {
    ...
    sellerContactDataRecord.setEMailAddress(
        seller.getEmailAddress() == null ? null : seller.getEmailAddress().getAddress());
    ...
    if (contactPerson != null
        && contactPerson.getEmailAddress() != null
        && contactPerson.getEmailAddress().getAddress() != null) {
  ...
  private InvoiceSumGroup invoiceSumGroup(
      BigDecimal exTax, BigDecimal tax, BigDecimal inTax, String currency) {
  ...
  private ItemEntry newItem(InvoiceLine line) {
    ...
    // todo
    //        if (line.getSaleOrderLine() != null && line.getSaleOrderLine().getContactPartner() !=
    // null) {
    //            String contactPerson =
    // line.getSaleOrderLine().getContactPartner().getSimpleFullName();
    ...
    BigDecimal taxRate = line.getTaxRate(); // 0.2 -> 20%
