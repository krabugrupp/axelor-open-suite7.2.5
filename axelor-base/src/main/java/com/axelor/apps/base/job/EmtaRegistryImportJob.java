...
public class EmtaRegistryImportJob implements Job {
  ...
  @Inject private IndustrySectorRepository sectorRepository;
  @Inject private PartnerTurnoverRepository turnoverRepository;
  ...
  public void execute(JobExecutionContext context) throws JobExecutionException {
    ...
    try (InputStream inputStream = new URL(urlParam).openConnection().getInputStream()) {
      ...
      for (int line = 1; line <= sheet.getLastRowNum(); line++) {
        ...
        int offset =
            row.getPhysicalNumberOfCells() == 10
                ? 1
                : 0; // workaround for different file format year 2019
